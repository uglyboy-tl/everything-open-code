...
const PLUGIN_DIR = path.join(os.homedir(), ".config", "opencode")
const CONFIG_PATH = path.join(PLUGIN_DIR, "peon-ping.json")
const STATE_PATH = path.join(PLUGIN_DIR, ".state.json")
const PAUSED_PATH = path.join(PLUGIN_DIR, ".paused")
const DEFAULT_PACKS_DIR = path.join(process.env.XDG_DATA_HOME || path.join(os.homedir(), ".local", "share"), "openpeon", "packs")
