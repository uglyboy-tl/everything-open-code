...
description: |
  ...
  基于维基百科的"AI 写作特征"综合指南。检测并修复以下模式：夸大的象征意义、宣传性语言、以 -ing 结尾的肤浅分析、模糊的归因、破折号过度使用、三段式法则、AI 词汇、否定式排比、过多的连接性短语、虚假代理、旁观者叙事、元评论。
allowed-tools:
  - Read
  - Write
  - Edit
...
6. **明确行动者** - 使用主动语态，指名谁在做某事，避免无生命事物执行人类动作
7. **删除元评论** - 让文章自己推进，不要预告结构或自我引用
...
**问题：** AI 喜欢给无生命的事物赋予人类动词，以避免指名行动者。这导致句子中没有人真正"做"任何事。

**改写前：**
> 投诉变成了修复。决策浮现出来。数据显示用户满意度上升。文化正在转变。市场奖励了这种做法。

**改写后：**
> 团队修复了投诉。管理层做出了决定。研究人员分析数据后发现用户满意度上升。人们的行为正在改变。消费者选择了这种产品。
...
**问题：** AI 喜欢用"没有人"、"人们"、"这是"等短语从远处观察，而不是让读者身临其境。

**改写前：**
> 没有人设计这个系统，它就这样演化了。人们倾向于低估复杂性。这是因为底层架构有问题。这就是为什么我们需要重新思考。

**改写后：**
> 你不会坐下来决定要设计一个有缺陷的系统，但它确实有缺陷。你可能低估了复杂性。底层架构有问题，所以我们需要重新思考。

**核心原则：** 把"你"放在句子里，而不是让"人们"或"没有人"做主语。
...
**问题：** AI 喜欢自我引用、预告文章结构，而不是让文章自己推进。
...
**改写前：**
> 提示：接下来我会解释为什么这很重要。在这一节中，我们将探讨三个原因。正如我们将看到的，答案其实很简单。

**改写后：**
> 这很重要，原因有三。首先……

**核心原则：** 删除所有"预告"，直接陈述内容。文章不需要告诉读者它要做什么。
...
**问题：** AI 喜欢用全称量词制造虚假权威感，但这些词往往不准确。

**需要避免的词：** 每个、总是、从不、所有人、没有人、永远、必然、全部、所有

**改写前：**
> 每个开发者都认同这个观点。这种做法总是有效的。没有人反对。

**改写后：**
> 大多数开发者认同这个观点。这种做法在测试中有效。反对意见很少。

**核心原则：** 用具体数据或限定范围代替全称量词。"80% 的用户"比"所有用户"更可信。
...
- ✓ **使用了"此外""然而"等连接词？** 考虑删除
- ✓ **三段式列举？** 改为两项或四项
- ✓ **无生命事物在执行人类动作？** 找到真正的行动者
- ✓ **"没有人""人们"做主语？** 把"你"放进句子
- ✓ **自我引用或结构预告？** 删除，直接陈述内容
- ✓ **使用了全称量词（每个、总是、从不）？** 用具体数据代替
...
**改写前（AI 味道）：**
> 新的软件更新作为公司致力于创新的证明。此外，它提供了无缝、直观和强大的用户体验——确保用户能够高效地完成目标。这不仅仅是一次更新，而是我们思考生产力方式的革命。行业专家认为这将对整个行业产生持久影响，彰显了公司在不断演变的技术格局中的关键作用。数据显示用户满意度上升。没有人设计这个系统，但每个人都在使用它。
...
- 删除了"数据显示"（虚假代理）- 改为"来自测试用户的早期反馈"
- 删除了"没有人设计这个系统"（旁观者叙事）- 直接陈述事实
- 添加了具体功能和具体反馈
...
本技能基于 [Wikipedia:Signs of AI writing](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing)，由 WikiProject AI Cleanup 维护，并参考了 [hardikpandya/stop-slop](https://github.com/hardikpandya/stop-slop)。

那里记录的模式来自对维基百科上数千个 AI 生成文本实例的观察。
