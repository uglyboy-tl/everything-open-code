...
1. Recent opencode sessions and their assistant work, from the raw trajectory
...
Trajectory database: `~/.local/share/opencode/opencode.db` (SQLite, read-only)
Memory files root: `.opencode/memory/`
...
3. Use bash to locate the database:
   ...
   - Treat the database path as read-only.
...
- Skills: Glob `{skill,skills}/**/SKILL.md` under the project `.opencode/` dir,
...
- Custom agents: Glob `{agent,agents}/**/*.md` under config directories.
- Plugins: Glob `.opencode/plugin*/**` for existing automation hooks.
...
- Skill - a reusable workflow or playbook. Write `SKILL.md` with YAML
  ...
  `.opencode/skills/<name>/` directory. Use a focused, imperative description so
  it is discoverable.
- Custom subagent - a bounded specialist role or investigation task suitable for
  delegation. Write `.opencode/agents/<name>.md` with frontmatter
  (`description`, optional `mode`, `model`, `tools`/permission) and the system
  prompt as the body.
- Command - a parameterized prompt for a recurring task. Write
  `.opencode/commands/<name>.md` with frontmatter (`description`, optional
  `agent`) and a template body using `$ARGUMENTS` / `$1` placeholders.
- Automation - Package recurring work as a command the user can re-run, or, only
  if clearly justified, a plugin lifecycle hook under `.opencode/plugins/`. Do
  not invent a scheduler. If a true schedule is needed, recommend it and explain
  the manual trigger instead.
...
- Write to the project `.opencode/` directory unless the user asked for global
  scope.
