...
**路径说明**：
- 本文档中的 `<RETRIEVE_CMD>` 表示检索脚本的执行命令
- 优先使用完整路径：`$HOME/.config/opencode/skills/retrieve/scripts/retrieve`
- 如果完整路径不可用，可尝试相对路径：`scripts/retrieve`
- 如果以上路径均不可用，可直接使用 `retrieve` 命令

**占位符说明**：
- `<RETRIEVE_CMD>`：代表检索命令，根据实际情况使用上述路径之一
...
```bash
# 语法
<RETRIEVE_CMD> exa [-t TYPE] [-c CATEGORY] [-n LIMIT] "query"

# 示例
<RETRIEVE_CMD> exa -t neural "React Hooks internals deep dive"
<RETRIEVE_CMD> exa -c "research paper" "Rust async programming patterns"
<RETRIEVE_CMD> exa -t fast "Node.js 20 new features"
```
...
```bash
# 搜索技术问题
<RETRIEVE_CMD> stackoverflow [-t "tag1;tag2"] [-n LIMIT] "query"

# 示例
<RETRIEVE_CMD> stackoverflow -t "python;pandas" "merge dataframes"
<RETRIEVE_CMD> stackoverflow -t "reactjs" "useEffect dependency array"
```
...
```bash
# 搜索技术讨论
<RETRIEVE_CMD> hackernews [-t TAG] [-p PERIOD] [-n LIMIT] "query"

# 示例
<RETRIEVE_CMD> hackernews -t story -p pastweek "AI coding"
<RETRIEVE_CMD> hackernews -t comment "Rust vs Go"
```
...
```bash
# 启动服务（每次使用前需重新启动）
bun scripts/cdp.mjs  # 或使用完整路径: $HOME/.config/opencode/skills/retrieve/scripts/cdp.mjs
```
...
<RETRIEVE_CMD> stackoverflow -t "next.js;webpack" "Module not found"
<RETRIEVE_CMD> exa -t neural "Next.js webpack troubleshooting"
```
